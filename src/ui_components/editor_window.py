...
class EditorWindow(ViewComponent):
    def __init__(self, tracker):
        ...
        self.previous_page = constants.page_map["pattern"]
    ...
    def handle_select(self):
        ...
        sel_track_index = self.tracker.pages[pmap["pattern"]].cursor_x
        if self.cursor_y < 4:
            if self.cursor_x == 0:
                if step.notes[self.cursor_y] is None:
                    step.add_note(self.cursor_y, self.tracker.last_note, self.tracker.last_velocity)
                else:
    ...
    def update_view(self):
        ...
        if self.tracker.page == pmap["detail"]:
            render_queue.appendleft(["rect", themeing.CURSOR_COLOR, self.x_pos, self.y_pos, self.w, self.h, 2])
        else:
            render_queue.appendleft(["rect", themeing.LINE_16_HL_BG, self.x_pos, self.y_pos, self.w, self.h, 2])

        render_queue.appendleft(["rect", themeing.BG_COLOR, self.x_pos + 2, self.y_pos + 2,
                                 self.w - 4, display.detail_window_replace_bg_h, 0])

        if self.previous_page == pmap["pattern"]:
            ...
            cursor_x = self.tracker.pages[pmap["pattern"]].cursor_x
            cursor_y = self.tracker.pages[pmap["pattern"]].cursor_y
            tr = f"{cursor_x}"
            cursor_col = themeing.CURSOR_COLOR if self.tracker.page == pmap["detail"] else themeing.BG_SEP
